...
int main (void) {
  ...
  uint32_t btns = 0;  
	...
	#ifdef __cplusplus
	volatile uint32_t _cpp = __cplusplus;
	#endif
	#ifdef __TARGET_ARCH_THUMB
	volatile uint32_t _target_arch_thumb = __TARGET_ARCH_THUMB;
	#endif
	...
	uint32_t stdc = __STDC__;
	uint32_t armcc_version = __ARMCC_VERSION;
	uint32_t stdc_version = __STDC_VERSION__;
